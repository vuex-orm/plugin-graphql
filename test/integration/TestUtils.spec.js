import { setupMockData, User, Post } from 'test/support/mock-data'
...
describe('TestUtils', () => {
  ...
  it('allows to mock a action without options', async () => {
    mock('fetch').for(User).andReturn(() => userData);
  ...
  it('allows to mock a destroy', async () => {
    mock('destroy', { id: 42 }).for(User).andReturn(userData);
  ...
  it('allows to mock a persist', async () => {
    mock('persist', { id: 42 }).for(User).andReturn(userData);
  ...
  it('allows to mock a push', async () => {
    mock('push').for(User).andReturn(userData);
  ...
  it('allows to mock a custom query', async () => {
    mock('query', { name: 'example' }).for(User).andReturn(userData);

    let result;
    const request = await recordGraphQLRequest(async () => {
      result = await User.query({ name: 'example', filter: { test: 42 } });
    }, true);
