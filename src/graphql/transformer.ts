...
import { clone, downcaseFirstLetter, isPlainObject, pluralize, singularize } from '../support/utils'
...
export default class Transformer {
  ...
  public static transformOutgoingData(
    ...
    Object.keys(data).forEach((key) => {
      ...
      if (
        !isRecursion &&
        this.shouldIncludeOutgoingField((recursiveCall as boolean) && read, key, value, model, whitelist)
      ) {
  ...
  public static shouldIncludeOutgoingField(
    ...
    // Ignore skiped fields
    if (model.baseModel.skipFields && model.baseModel.skipFields.indexOf(fieldName) >= 0) return false
   ...
   */
  private static addRecordForRecursionDetection(records: Map<string, Array<string>>, record: ORMModel): void {
